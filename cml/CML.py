        ...
        def getCML(self, neighborhood, function, coupling, nit, snapshot, parameters=[]):
                ...
                if(function.__name__ == 'onebyfMap'):
                        ...
                        for ny, nx in neighborhood:
                                outputMat += (coupling/len(neighborhood)) * function(self.mat[np.roll(np.arange(rows), -nx).tolist()][np.roll(np.arange(cols), -ny).tolist()], parameters, mat_shape, nit, snapshot)
                else:
